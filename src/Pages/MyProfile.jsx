...
const PhoneInput = ({ id, value, onChange, error }) => (
  <Field label="Phone number" htmlFor={id} error={error}>
    <div className="flex">
      <span
        className="flex items-center px-3.5 rounded-l-xl border border-r-0 border-gray-200
...
const MyProfile = () => {
  ...
  const loadProfiles = useCallback(
    async (preferredId) => {
      ...
      const nextId =
        preferredId && list.some((p) => p.id === preferredId) ? preferredId : (list[0]?.id ?? null);
        ...
        {tab === "profile" ? (
              ...
              <>
                <div className="mb-6 space-y-3">
                  ...
                  {profiles.length > 1 && (
                    <button
                      ...
                      className={Btn.danger("gap-2 text-xs py-2 px-3")}
                    >
                      <TrashIcon size={10} /> Delete selected profile
                    </button>
                  )}
                  <p className="text-xs text-gray-500">
                    Create separate profiles for different roles or CVs. When applying, you can
                ...
                <Field label="Profile name" htmlFor="profileName" error={errors.profileName}>
                  <input
                    ...
                    className={inputCls(errors.profileName)}
              ...
              <>
                ...
                <Field label="Company name" htmlFor="coName" error={errors.coName}>
                  <input
                    ...
                    className={inputCls(errors.coName)}
                  />
                </Field>
                ...
                <Field label="Contact email" htmlFor="coContactEmail" error={errors.coContactEmail}>
                  <input
                    id="coContactEmail"
                    type="email"
                    value={coContactEmail}
                    onChange={(e) => setCoContactEmail(e.target.value)}
                    className={inputCls(errors.coContactEmail)}
                  />
                </Field>
                <PhoneInput
                  id="coPhone"
          ...
          <form onSubmit={handlePasswordSubmit} noValidate>
            <p className="text-sm text-gray-500 mb-6">
              Choose a strong password with upper and lower case letters, a number, and a special
              character.
            </p>
            <PwdField
              id="currentPassword"
