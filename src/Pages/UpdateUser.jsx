...
const UpdateUser = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // CV uploaded separately if a new file was selected
      if (cv) {
  ...
  return (
    <FormCard title="Update your profile" onSubmit={handleSubmit}>
      ...
      <Field label="Age" htmlFor="age" error={errors.age}>
        <input id="age" type="text" placeholder="e.g. 25"
          value={age} onChange={(e) => setAge(e.target.value)} maxLength={2} className={inputCls(errors.age) + " w-24"} />
      </Field>
          ...
          {["Male", "Female"].map((s) => (
            <label key={s} className="flex items-center gap-2 cursor-pointer text-sm">
              <input type="radio" name="sex" value={s} checked={sex === s}
                onChange={(e) => setSex(e.target.value)} className="accent-blue-600" />
              {s}
            </label>
          ))}
        </div>
